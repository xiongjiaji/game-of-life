...
The application is a very simple online version of [Conway's 'game of life'](http://en.wikipedia.org/wiki/Conway's_Game_of_Life). To see what the game does, run `mvn install` as described above, thengo to the gameoflife-web directory and run `mvn org.mortbay.jetty:maven-jetty-plugin:run`. The application will be running on http://localhost:8080.
